...
PrintMessage(DmtxRegion *reg, DmtxMessage *msg, UserOptions *opt)
{
   ...
   if (opt->verbose == DmtxTrue && msg->array != NULL)
   {
      printf("DmtxMessage Contents:\n");
      printf("  arraySize: %zu\n", msg->arraySize);
      printf("  codeSize: %zu\n", msg->codeSize);
      ...
      fprintf(stdout, "Parsed data contents:\n");
      ...
      int rows = dmtxGetSymbolAttribute(DmtxSymAttribDataRegionRows, reg->sizeIdx);
      int cols = dmtxGetSymbolAttribute(DmtxSymAttribDataRegionCols, reg->sizeIdx);
      for (int i = 0; i < rows; i++)
      {
         for (int j = 0; j < cols; j++)
         {
            printf("%02X ", msg->array[i * cols + j]);
         }
         printf("\n");
      }
      printf("\n");
